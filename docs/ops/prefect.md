...
| `prefect-server` | Prefect API + UI (дашборд) | внутр. 4200 (через прокси) |
| `prefect-proxy` | nginx — basic-auth перед Prefect UI | **0.0.0.0:4200** (наружу) |
...
Открыть: **http://109.120.183.147:4200** → nginx basic-auth, нативный диалог браузера с
отдельными полями **Username** и **Password** (`admin` / пароль). Креды — в `nginx/.htpasswd`
(gitignored, не в репозитории).

Сменить пароль:
```bash
printf "admin:$(openssl passwd -apr1 'НОВЫЙ_ПАРОЛЬ')\n" > nginx/.htpasswd
docker compose restart prefect-proxy
```
...
- `nginx/.htpasswd` (логин/пароль дашборда) — gitignored.
