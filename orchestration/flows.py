...
JOBS: list[dict] = [
    ...
    # рассылка кандидатам из tg_vacancies — LIVE: реальные ЛС эйчарам (с PDF-резюме + папка «Отклики»).
    # Шлют только аккаунты с feat.tg_channels + tg_user_session. Убрать "--live" -> вернётся DRY.
