...
CATS = ("general","analytics","python","go","java","backend","frontend","ds_ml","devops","mobile","qa","gamedev","product","remote","design")
SYS = (
    ...
    async with httpx.AsyncClient(headers={"user-agent": "Mozilla/5.0"}, follow_redirects=True) as cl:
        for ch in channels:
            ...
            for pid, dt, text, links in _parse(r.text):
                ...
                cur.execute(
                    "INSERT INTO tg_vacancies (channel,post_id,posted_at,text,is_vacancy,category,title,contact,contact_type,salary,remote,region,post_url,langs) "
                    ...
                    (ch, pid, dt, text[:4000], cat, (d.get("title") or "")[:200], contact,
                     ...
                     (d.get("region") or "")[:60], f"https://t.me/{ch}/{pid}", _norm_langs(d.get("langs"))))
                stored += 1
